...
## Spending Multiple Notes

You can spend **2 notes** in a single withdrawal (Withdraw2):
...
────────────────
Withdrawal:    0.4 ETH
Change Note:   0.1 ETH (minus fees)
```

This is useful when you have multiple small notes and want to make a larger withdrawal.

## Fee Structure

:::info[Testnet]
Fees are configurable. Solver availability on testnet is limited.
:::

| Fee | When |
|-----|------|
| **Relay Fee** | Always (covers gas) |
| **Solver Fee** | Crosschain only |

The relay fee is dynamic based on gas prices.
...
| **Solver doesn't fill (crosschain)** | Withdrawal delayed | Intent expires → refund commitment created |
...
- **[Crosschain Architecture](/concepts/cross-chain)** — How OIF works
