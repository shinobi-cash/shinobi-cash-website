...
Shinobi Cash uses **Hyperlane** for cross-chain oracle messaging. The `HyperlaneOracle` contract implements both oracle roles:

- Receives messages via Hyperlane Mailbox
- Marks payloads as proven when messages arrive
- Submits proofs to destination chains

This provides a decentralized, permissionless bridge for intent proofs without relying on a centralized relay service.
