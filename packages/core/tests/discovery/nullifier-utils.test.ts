...
describe('nullifier-utils', () => {
  describe('deriveNullifier', () => {
    ...
    it('should produce different nullifiers for different accountKey', () => {
      const nullifier0 = deriveNullifier(TEST_ACCOUNT_KEY, TEST_POOL_ADDRESS, TEST_CHAIN_ID, 0, 0);
      const nullifier1 = deriveNullifier(TEST_ACCOUNT_KEY + 1n, TEST_POOL_ADDRESS, TEST_CHAIN_ID, 0, 0);
    ...
    it('should produce different nullifiers for different poolAddress', () => {
      const nullifier0 = deriveNullifier(TEST_ACCOUNT_KEY, TEST_POOL_ADDRESS, TEST_CHAIN_ID, 0, 0);
      const nullifier1 = deriveNullifier(TEST_ACCOUNT_KEY, '0x9999999999999999999999999999999999999999', TEST_CHAIN_ID, 0, 0);
    ...
    it('should produce different nullifiers for different chainId', () => {
      ...
      const nullifierArbitrum = deriveNullifier(TEST_ACCOUNT_KEY, TEST_POOL_ADDRESS, arbitrumChainId, 0, 0);
  ...
  describe('deriveAndHashNullifier', () => {
    ...
    it('should be equivalent to deriveNullifier + hashNullifier', () => {
      ...
      const actualHash = deriveAndHashNullifier(TEST_ACCOUNT_KEY, TEST_POOL_ADDRESS, TEST_CHAIN_ID, 0, 0);

      expect(actualHash).toBe(expectedHash);
    });

    it('should produce unique hashes for different positions', () => {
      const hash00 = deriveAndHashNullifier(TEST_ACCOUNT_KEY, TEST_POOL_ADDRESS, TEST_CHAIN_ID, 0, 0);
      const hash01 = deriveAndHashNullifier(TEST_ACCOUNT_KEY, TEST_POOL_ADDRESS, TEST_CHAIN_ID, 0, 1);
      const hash10 = deriveAndHashNullifier(TEST_ACCOUNT_KEY, TEST_POOL_ADDRESS, TEST_CHAIN_ID, 1, 0);
    ...
    it('should be deterministic', () => {
      const hash1 = deriveAndHashNullifier(TEST_ACCOUNT_KEY, TEST_POOL_ADDRESS, TEST_CHAIN_ID, 5, 3);
      const hash2 = deriveAndHashNullifier(TEST_ACCOUNT_KEY, TEST_POOL_ADDRESS, TEST_CHAIN_ID, 5, 3);
    ...
    it('should produce different hashes for different chainId', () => {
      const arbitrumChainId = '421614';
      const baseChainId = '84532';
      const hashArbitrum = deriveAndHashNullifier(TEST_ACCOUNT_KEY, TEST_POOL_ADDRESS, arbitrumChainId, 0, 0);
      const hashBase = deriveAndHashNullifier(TEST_ACCOUNT_KEY, TEST_POOL_ADDRESS, baseChainId, 0, 0);
  ...
  describe('deriveDepositPrecommitment', () => {
    it('should return a string', () => {
      const precommitment = deriveDepositPrecommitment(TEST_ACCOUNT_KEY, TEST_POOL_ADDRESS, TEST_CHAIN_ID, 0);

      expect(typeof precommitment).toBe('string');
    });

    it('should produce different precommitments for different depositIndex', () => {
      const precommitment0 = deriveDepositPrecommitment(TEST_ACCOUNT_KEY, TEST_POOL_ADDRESS, TEST_CHAIN_ID, 0);
      const precommitment1 = deriveDepositPrecommitment(TEST_ACCOUNT_KEY, TEST_POOL_ADDRESS, TEST_CHAIN_ID, 1);
    ...
    it('should produce different precommitments for different accountKey', () => {
      const precommitment0 = deriveDepositPrecommitment(TEST_ACCOUNT_KEY, TEST_POOL_ADDRESS, TEST_CHAIN_ID, 0);
      const precommitment1 = deriveDepositPrecommitment(TEST_ACCOUNT_KEY + 1n, TEST_POOL_ADDRESS, TEST_CHAIN_ID, 0);
    ...
    it('should be deterministic', () => {
      const precommitment1 = deriveDepositPrecommitment(TEST_ACCOUNT_KEY, TEST_POOL_ADDRESS, TEST_CHAIN_ID, 5);
      const precommitment2 = deriveDepositPrecommitment(TEST_ACCOUNT_KEY, TEST_POOL_ADDRESS, TEST_CHAIN_ID, 5);
    ...
    it('should handle sequential deposit indices', () => {
      ...
      for (let i = 0; i < 10; i++) {
        precommitments.push(deriveDepositPrecommitment(TEST_ACCOUNT_KEY, TEST_POOL_ADDRESS, TEST_CHAIN_ID, i));
      }
    ...
    it('should produce different precommitments for different chainId (per-chain index space)', () => {
      ...
      const precommitmentArbitrum = deriveDepositPrecommitment(TEST_ACCOUNT_KEY, TEST_POOL_ADDRESS, arbitrumChainId, 0);
      const precommitmentBase = deriveDepositPrecommitment(TEST_ACCOUNT_KEY, TEST_POOL_ADDRESS, baseChainId, 0);

      expect(precommitmentArbitrum).not.toBe(precommitmentBase);
    });

    it('should accept chainId as number, bigint, or string', () => {
      const precommitmentNumber = deriveDepositPrecommitment(TEST_ACCOUNT_KEY, TEST_POOL_ADDRESS, 421614, 0);
      const precommitmentBigint = deriveDepositPrecommitment(TEST_ACCOUNT_KEY, TEST_POOL_ADDRESS, 421614n, 0);
      const precommitmentString = deriveDepositPrecommitment(TEST_ACCOUNT_KEY, TEST_POOL_ADDRESS, '421614', 0);
