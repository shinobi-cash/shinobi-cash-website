  ...
  describe("extendAllTrees", () => {
    ...
    describe("1:1 withdrawal extension", () => {
      it("should extend tree with 1:1 withdrawal", () => {
        ...
        const withdrawalNode = updatedTree.root.children.find(
          (c) => c.note.noteType === "withdrawal"
        )!;
        expect(withdrawalNode).toBeDefined();
        expect(withdrawalNode.note.amount).toBe("0"); // Terminal notes have no remaining balance
        expect((withdrawalNode.note as WithdrawalNote).withdrawnAmount).toBe(
          toEther(0.3).toString()
        ); // Withdrawn amount
      ...
      it("should update nullifier map after extension", () => {
        ...
        const oldNullifier = deriveAndHashNullifier(
          TEST_ACCOUNT_KEY,
          TEST_POOL_ADDRESS,
          TEST_CHAIN_ID,
          0,
        ...
        const newNullifier = deriveAndHashNullifier(
          TEST_ACCOUNT_KEY,
          TEST_POOL_ADDRESS,
          TEST_CHAIN_ID,
          0,
    ...
    describe("Withdraw2 extension", () => {
      it("should merge two trees with Withdraw2", () => {
        ...
        const primaryChangeNode = primaryTree.root.children.find(
          (c) => c.note.noteType === "change"
        )!;
    ...
    describe("ragequit extension", () => {
      it("should mark note as ragequit with matching commitment", () => {
        ...
        const nullifier = deriveAndHashNullifier(
          TEST_ACCOUNT_KEY,
          TEST_POOL_ADDRESS,
          TEST_CHAIN_ID,
          0,
    ...
    describe("multiple trees", () => {
      it("should extend multiple trees independently", () => {
        ...
        const change0 = result.updatedTrees
          .get(chainKey0)!
          .root.children.find((c) => c.note.noteType === "change")!.note as ChangeNote;
        const change1 = result.updatedTrees
          .get(chainKey1)!
          .root.children.find((c) => c.note.noteType === "change")!.note as ChangeNote;
    ...
    describe("immutability", () => {
      it("should not mutate original nullifier map", () => {
        ...
        extendAllTrees(trees, nullifierMap, activityIndex, TEST_CRYPTO, TEST_POOL_ADDRESS);
