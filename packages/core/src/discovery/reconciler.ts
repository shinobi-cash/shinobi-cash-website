    ...
    traverseTree(tree, (node) => {
      ...
      if (note.noteType === 'withdrawalIntent') {
        ...
        if (fillActivity && isCrosschainWithdrawalFill(fillActivity)) {
          const wasReconciled = reconcileWithdrawalIntentWithFill(withdrawalIntent, fillActivity, node, childrenToAdd);
        ...
        const refundActivity = activityIndex.withdrawalRefundsByOrderId.get(withdrawalIntent.orderId);
        if (refundActivity) {
          const wasReconciled = reconcileWithdrawalIntentWithRefund(withdrawalIntent, node, childrenToAdd);
      ...
      // Handle deposit intent notes
        ...
        if (fillActivity && isCrosschainDepositFill(fillActivity)) {
          const wasReconciled = reconcileDepositIntentWithFill(depositIntent, fillActivity, node, childrenToAdd, result);
          if (wasReconciled && !addedActivityTxHashes.has(fillActivity.txHash)) {
            result.matchedActivities.push(fillActivity);
            addedActivityTxHashes.add(fillActivity.txHash);
          }
        ...
        if (refundActivity) {
          const wasReconciled = reconcileDepositIntentWithRefund(depositIntent, node, childrenToAdd);
          if (wasReconciled && !addedActivityTxHashes.has(refundActivity.txHash)) {
            result.matchedActivities.push(refundActivity);
            addedActivityTxHashes.add(refundActivity.txHash);
          }
